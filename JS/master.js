/* =========================
   ...
   document.addEventListener('DOMContentLoaded', () => {
    ...
    /* =========================
       CLIENTES + ANIVERSÁRIOS + FOLHA
       ========================= */
    (function () {
      // ===== CLIENTES (lista resumida + aniversariantes)
      ...
      function renderClients() {
        ...
        if (!clients || clients.length === 0) {
          ...
          if (bdayBar) bdayBar.hidden = true;
          return;
        }
        ...
        clients
          ...
          .forEach(c => {
            ...
            const ageBadge = a !== null ? `<span class="badge-age">${a} anos</span>` : '';
  
            const tdNome  = cell(c.nome, 'Nome');
            const tdFone  = cell(c.fone, 'Celular');
            const tdEmail = cell(c.email || '—', 'E-mail');
            const tdNiver = cell(`${fmtDate(c.niver)} ${ageBadge}`, 'Aniversário');
  
            const tdActions = document.createElement('td');
            tdActions.setAttribute('data-actions', '');
            tdActions.innerHTML = `
              <button class="btn ghost btn-sm js-edit-client"><i class="fa-regular fa-pen-to-square"></i> Editar</button>
              <button class="btn ghost btn-sm js-del-client"><i class="fa-regular fa-trash-can"></i> Remover</button>
            `;
      ...
      function updateBirthdayBar() {
        if (!bdayBar || !bdayMsg) return;
        const today = clients.filter(c => isToday(c.niver));
        if (today.length) {
          bdayMsg.textContent = `Aniversariantes de hoje: ${today.map(t => t.nome).join(', ')}`;
          bdayBar.hidden = false;
        } else {
          bdayBar.hidden = true;
        }
      }
      ...
      btnNotifyAll?.addEventListener('click', () => {
        alert('🎉 Mensagem enviada aos aniversariantes (simulação)!');
      });
