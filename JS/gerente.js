...
document.addEventListener("DOMContentLoaded", () => {
  ...
  relButtons.forEach(btn => {
    btn.addEventListener("click", (ev) => {
      ...
      const raw = (btn.getAttribute("data-view") || "").trim(); // ex: "rel-pagamentos"
      if (!raw) return console.warn("colab-item sem data-view");

      // tenta id literal, depois com prefixo 'view-'
      let targetId = raw;
      if (!document.getElementById(targetId) && document.getElementById("view-" + raw)) {
        targetId = "view-" + raw;
      }
      ...
      if (targetId && document.getElementById(targetId)) {
        ...
        console.warn("View não encontrada para:", raw, "-> tentou:", targetId);
      }
    });
